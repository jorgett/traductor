# Script para iniciar Traductor Chat en modo DESARROLLO
Write-Host "🛠️  Iniciando Traductor Chat en modo DESARROLLO..." -ForegroundColor Green
Write-Host ""
...
if ($isFirstTime) {
    ...
    if (-not $modelsExist) {
        ...
        $downloadModels = Read-Host "¿Descargar modelos básicos (en-es, es-en)? (Y/n)"
        if ($downloadModels -ne "n" -and $downloadModels -ne "N") {
            Write-Host "📥 Descargando modelos básicos..." -ForegroundColor Blue
            try {
                ...
                Write-Host "⚠️  Error descargando modelos. Continuando sin ellos..." -ForegroundColor Yellow
            }
        }
    }
}

Write-Host "🐳 Construyendo y ejecutando contenedores en modo desarrollo..." -ForegroundColor Blue
...
if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "🎉 Traductor Chat está ejecutándose en modo desarrollo:" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "📝 En modo desarrollo, puedes editar los archivos y reiniciar el contenedor para ver cambios." -ForegroundColor Yellow
